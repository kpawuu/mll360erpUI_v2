  ...
  <div class="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
    ...
    <div class="mb-8">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Shipment Types Management</h1>
                    <p class="text-gray-600 dark:text-gray-400 text-lg">Manage different types of shipments for your operations</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Search & Filter Shipment Types</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Find and manage your shipment types</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-2">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                            {{ filteredShipmentTypes.length }} types
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div class="lg:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Search Shipment Types</label>
                        <div class="relative">
          ...
          <input 
            ...
            type="text" 
                                placeholder="Search by type name or short code..."
                            ...
                            >
                            <div v-if="shipmentTypeStore.loading" class="absolute inset-y-0 right-0 flex items-center pr-4">
        ...
        >
                            <div v-if="shipmentTypeStore.loading" class="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent mr-2"></div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            <span class="text-sm text-gray-600 dark:text-gray-400">
                                Found {{ filteredShipmentTypes.length }} result{{ filteredShipmentTypes.length !== 1 ? 's' : '' }}
                            </span>
                            <button 
                                @click="clearFilters"
    ...
    <div v-if="shipmentTypeStore.loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="n in 6" :key="n" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 animate-pulse">
        ...
        <!-- Shipment Types Display -->
        <div v-else>
            <!-- Empty State -->
            <div v-if="filteredShipmentTypes.length === 0" class="flex flex-col items-center justify-center py-20">
                ...
                <p class="text-gray-600 dark:text-gray-400 text-center max-w-md mb-8 text-lg">
                    {{ searchQuery ? 'Try adjusting your search criteria or clear the filters.' : 'Get started by adding your first shipment type to organize your operations.' }}
                </p>
            ...
            <div v-else-if="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div v-for="shipmentType in filteredShipmentTypes" :key="shipmentType.id" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 hover:border-gray-300 dark:hover:border-gray-500 transition-colors duration-200">
        <div class="flex justify-between items-start mb-4">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Code: {{ shipmentType.short_code }}</p>
                        ...
                        <div class="flex space-x-1">
                            <button @click="editShipmentType(shipmentType)" class="p-1 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
              </svg>
            </button>
                            <button @click="deleteShipmentType(shipmentType.id)" class="p-1 text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                    ...
                    <div class="space-y-3 mb-4">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="text-gray-900 dark:text-white font-medium">{{ shipmentType.type }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500 dark:text-gray-400">Code:</span>
                            <span class="text-gray-900 dark:text-white font-medium">{{ shipmentType.short_code }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500 dark:text-gray-400">Created:</span>
                            <span class="text-gray-900 dark:text-white font-medium">{{ formatDate(shipmentType.date_created) }}</span>
          </div>
        </div>

        <div class="flex space-x-2">
                        <button @click="editShipmentType(shipmentType)" class="flex-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-3 py-2 rounded-lg text-sm hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors flex items-center justify-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                            </svg>
                            Edit Type
                        </button>
                        <button @click="deleteShipmentType(shipmentType.id)" class="flex-1 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 px-3 py-2 rounded-lg text-sm hover:bg-red-200 dark:hover:bg-red-800 transition-colors flex items-center justify-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                            </svg>
                            Delete
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex items-center space-x-4 mt-1 text-xs text-gray-500 dark:text-gray-400">
                                    <span>Created: {{ formatDate(shipmentType.date_created) }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center space-x-2">
                            <button @click="editShipmentType(shipmentType)" class="px-3 py-2 text-sm bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                                Edit
                            </button>
                            <button @click="deleteShipmentType(shipmentType.id)" class="px-3 py-2 text-sm bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 rounded-lg hover:bg-red-200 dark:hover:bg-red-800 transition-colors flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                </svg>
                                Delete
            ...
            <div class="relative bg-white rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-800">
                ...
                <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-t-xl">
                    <h3 class="text-xl font-semibold text-white flex items-center">
                        ...
                        {{ showEditModal ? 'Edit Shipment Type' : 'Create New Shipment Type' }}
                    </h3>
                ...
                <div class="p-6 space-y-6">
                    <form @submit.prevent="saveShipmentType">
                        <!-- Shipment Type Information Section -->
                        <div class="space-y-6">
                            <div>
                                ...
                                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Define the type and code for this shipment category.</p>
                            </div>
        
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <!-- Type Name -->
          <div>
                                    <label for="type-name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Type Name</label>
            <input 
                                        id="type-name"
            ...
            >
                                ...
                                <!-- Short Code -->
          <div>
                                    <label for="short-code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Short Code</label>
            <input 
                                        id="short-code"
            ...
            <button 
              ...
              :disabled="shipmentTypeStore.loading"
                    ...
                    >
                        <div v-if="shipmentTypeStore.loading" class="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                        {{ showEditModal ? 'Update Shipment Type' : 'Create Shipment Type' }}
                    </button>
                </div>
            </div>
        </div>
...
const selectedShipmentType = ref<ShipmentType | null>(null)
...
const saveShipmentType = async () => {
  try {
    if (showEditModal.value && editingShipmentType.value) {
      ...
      successMessage.value = 'Shipment type updated successfully!'
    } else {
      await shipmentTypeStore.createShipmentType(shipmentTypeForm.value)
      showSuccessModal.value = true
      successMessage.value = 'Shipment type created successfully!'
    }
    closeModal()
    ...
    showSuccessModal.value = true
    successMessage.value = 'Failed to save shipment type. Please try again.'
  }
}

const deleteShipmentType = async (id: number) => {
  confirmMessage.value = 'Are you sure you want to delete this shipment type? This action cannot be undone.'
  confirmAction.value = async () => {
    try {
      await shipmentTypeStore.deleteShipmentType(id)
      showSuccessModal.value = true
      successMessage.value = 'Shipment type deleted successfully!'
      ...
      showSuccessModal.value = true
      successMessage.value = 'Failed to delete shipment type. Please try again.'
      showConfirmModal.value = false
    }
  }
