          ...
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ packageSize.package_type?.type || 'Package Type' }}</p>
                        ...
                        <div class="flex space-x-1">
                            <button @click="editPackageSize(packageSize)" class="p-1 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
              </svg>
            </button>
                            <button @click="deletePackageSize(packageSize.id)" class="p-1 text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                    ...
                    <div class="space-y-3 mb-4">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="text-gray-900 dark:text-white font-medium">{{ packageSize.size }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
            <span class="text-gray-500 dark:text-gray-400">Description:</span>
                            <span class="text-gray-900 dark:text-white font-medium">{{ packageSize.description || 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
            ...
            <span class="text-gray-500 dark:text-gray-400">Created:</span>
                            <span class="text-gray-900 dark:text-white font-medium">{{ formatDate(packageSize.date_created) }}</span>
          </div>
        </div>

        <div class="flex space-x-2">
                        <button @click="editPackageSize(packageSize)" class="flex-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-3 py-2 rounded-lg text-sm hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors flex items-center justify-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                            </svg>
                            Edit Size
                        </button>
                        <button @click="deletePackageSize(packageSize.id)" class="flex-1 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 px-3 py-2 rounded-lg text-sm hover:bg-red-200 dark:hover:bg-red-800 transition-colors flex items-center justify-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                            </svg>
                            Delete
            ...
            <div v-else class="space-y-4">
                <div v-for="packageSize in filteredPackageSizes" :key="packageSize.id" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 hover:border-gray-300 dark:hover:border-gray-500 transition-colors duration-200">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-2">
                            <button @click="editPackageSize(packageSize)" class="px-3 py-2 text-sm bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                                Edit
                            </button>
                            <button @click="deletePackageSize(packageSize.id)" class="px-3 py-2 text-sm bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 rounded-lg hover:bg-red-200 dark:hover:bg-red-800 transition-colors flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                </svg>
                                Delete
      ...
      <div class="relative bg-white rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-800">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-t-xl">
          <h3 class="text-xl font-semibold text-white flex items-center">
            ...
            {{ showEditModal ? 'Edit Package Size' : 'Create New Package Size' }}
        </h3>
        ...
        <div class="p-6 space-y-6">
          <form @submit.prevent="savePackageSize">
            ...
            <div class="space-y-6">
              <div>
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Define the size and package type for this package category.</p>
              </div>
          ...
          <div>
            ...
            >
              ...
              <option v-for="packageType in packageTypes" :key="packageType.id" :value="packageType.id">
                ...
                <!-- Size Name -->
          <div>
                  <label for="size-name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Size Name</label>
            <input 
                    id="size-name"
            ...
            >
                  <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    Enter a descriptive name for this package size.
                  </p>
                </div>
          </div>
              ...
              <!-- Description -->
          <div>
                <label for="size-description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Description</label>
            <textarea 
                  id="size-description"
              v-model="packageSizeForm.description" 
                  rows="4"
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 transition-colors"
                  placeholder="Enter a detailed description of this package size..."
            ></textarea>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Provide a clear description to help identify and categorize this package size.
                </p>
              </div>
            </div>
          </form>
        ...
        <div class="flex items-center justify-end p-6 border-t border-gray-200 dark:border-gray-700 space-x-3">
            ...
            >
            <div v-if="packageSizeStore.loading" class="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
            {{ showEditModal ? 'Update Package Size' : 'Create Package Size' }}
            </button>
          </div>
      </div>
    </div>
...
const savePackageSize = async () => {
  try {
    if (showEditModal.value && editingPackageSize.value) {
      ...
      successMessage.value = 'Package size updated successfully!'
    } else {
      await packageSizeStore.createPackageSize(packageSizeForm.value)
      showSuccessModal.value = true
      successMessage.value = 'Package size created successfully!'
    }
    closeModal()
    // Refresh the data after create/update
    await loadPackageSizes()
  } catch (error) {
    console.error('Failed to save package size:', error)
    showSuccessModal.value = true
    successMessage.value = 'Failed to save package size. Please try again.'
  }
}

const deletePackageSize = async (id: number) => {
  confirmMessage.value = 'Are you sure you want to delete this package size? This action cannot be undone.'
  confirmAction.value = async () => {
    try {
      await packageSizeStore.deletePackageSize(id)
      showSuccessModal.value = true
      successMessage.value = 'Package size deleted successfully!'
      ...
      showSuccessModal.value = true
      successMessage.value = 'Failed to delete package size. Please try again.'
      showConfirmModal.value = false
    }
  }
