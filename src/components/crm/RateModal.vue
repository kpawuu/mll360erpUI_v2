    ...
    <div class="relative w-full max-w-4xl max-h-full">
      <div class="relative bg-white rounded-lg border border-gray-200 dark:border-gray-700 dark:bg-gray-800">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-t-xl">
          ...
          <button @click="handleClose" class="text-white bg-transparent hover:bg-white/20 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center transition-colors">
...
const fetchExistingRates = async (opportunityId: number) => {
  try {
    ...
    const rates = await logisticsContractRatesStore.fetchRatesByOpportunity(opportunityId)
    existingRates.value = rates || []
  } catch (err: any) {
...
const deleteExistingRate = async (rateId: number) => {
  ...
  try {
    ...
    // Remove from local existingRates array
    existingRates.value = existingRates.value.filter(rate => rate.id !== rateId)
  } catch (err: any) {
...
const handleClose = () => {
  // Clear the existing rates when modal is closed
  existingRates.value = []
  emit('close')
}
